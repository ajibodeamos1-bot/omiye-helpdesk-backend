...
router.get('/', auth, async (req, res) => {
  try {
    const { status, priority, search, department, page=1, limit=20 } = req.query;
    ...
    const countRes = await pool.query(`SELECT COUNT(*) FROM tickets t ${whereClause}`, params);
    ...
    res.json({ tickets:result.rows, total:parseInt(countRes.rows[0].count), page:parseInt(page), pages:Math.ceil(countRes.rows[0].count/limit) });
...
router.post('/', auth, requireRole('care_rep','ict_staff','ict_manager','finance_officer','super_admin'), upload.array('attachments',5), async (req, res) => {
  const { subject, description, category, priority, branch, affected_staff, department='ict' } = req.body;
  ...
  try {
    ...
    const ticketRes = await client.query(
      `INSERT INTO tickets (subject,description,category,priority,branch,affected_staff,created_by,sla_deadline,department)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [subject,description,category,priority,branch,affected_staff||null,req.user.id,slaDeadline,department]
    );
    ...
    const creatorRes = await pool.query('SELECT full_name,email FROM users WHERE id=$1',[req.user.id]);
    const creator = creatorRes.rows[0];
    const deptRole = department==='finance' ? "'finance_officer','ict_manager'" : "'ict_staff','ict_manager'";
    const staffRes = await pool.query(`SELECT email FROM users WHERE role IN (${deptRole}) AND is_active=true`);
    for (const u of staffRes.rows) sendEmail(u.email, emailTemplates.ticketCreated(ticket,creator));
...
router.get('/stats/dashboard', auth, async (req, res) => {
  try {
    ...
    res.json({
      open:parseInt(open.rows[0].count), in_progress:parseInt(inprog.rows[0].count),
      resolved_today:parseInt(resolvedToday.rows[0].count), sla_breached:parseInt(slaBreach.rows[0].count),
      by_status:byStatus.rows, by_priority:byPriority.rows, by_category:byCategory.rows,
    });
  } catch (err) { console.error(err); res.status(500).json({ message:'Server error' }); }
});
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const commentsQ = (req.user.role==='care_rep'||req.user.role==='finance_officer')
      ? `SELECT cm.*,u.full_name AS author_name,u.role AS author_role,
           COALESCE(json_agg(att.*) FILTER (WHERE att.id IS NOT NULL),'[]') AS attachments
         FROM comments cm 
         LEFT JOIN users u ON cm.author_id=u.id
         LEFT JOIN attachments att ON att.comment_id=cm.id
         WHERE cm.ticket_id=$1 AND cm.is_internal=false
         GROUP BY cm.id,u.full_name,u.role ORDER BY cm.created_at`
      : `SELECT cm.*,u.full_name AS author_name,u.role AS author_role,
           COALESCE(json_agg(att.*) FILTER (WHERE att.id IS NOT NULL),'[]') AS attachments
         FROM comments cm
         LEFT JOIN users u ON cm.author_id=u.id
    ...
    const [comments, attachments, audit] = await Promise.all([
      pool.query(commentsQ, [req.params.id]),
      pool.query(
        `SELECT a.*,u.full_name AS uploaded_by_name FROM attachments a 
         LEFT JOIN users u ON a.uploaded_by=u.id 
         WHERE a.ticket_id=$1 AND a.comment_id IS NULL ORDER BY a.created_at`,
        [req.params.id]
      ),
      pool.query(
        'SELECT al.*,u.full_name AS user_name FROM audit_logs al LEFT JOIN users u ON al.user_id=u.id WHERE al.ticket_id=$1 ORDER BY al.created_at',
        [req.params.id]
      ),
    ]);
...
// PUT /api/tickets/:id - update status + assignment + optional simultaneous comment
router.put('/:id', auth, requireRole('ict_staff','ict_manager','finance_officer','super_admin'), async (req, res) => {
  ...
  try {
    ...
    if (assigned_to!==undefined && assigned_to!==(old.assigned_to||'')) {
      ...
      const aName = assigned_to ? (await pool.query('SELECT full_name FROM users WHERE id=$1',[assigned_to])).rows[0]?.full_name : 'Unassigned';
      auditActions.push({ action:`Assigned to: ${aName}`, old_value:old.assigned_to, new_value:assigned_to });
    }
    ...
    // Save comment simultaneously if provided
    if (comment?.trim()) {
    ...
    if (status && status!==old.status) {
      ...
      const creator=creatorRes.rows[0], updater=updaterRes.rows[0];
      if (creator) {
        if (status==='resolved') sendEmail(creator.email, emailTemplates.ticketResolved(updatedTicket,updater,comment));
        else sendEmail(creator.email, emailTemplates.ticketUpdated(updatedTicket,updater,`Status: ${status.replace('_',' ')}`,comment));
      }
    }
...
// POST /api/tickets/:id/comments — with optional attachments
router.post('/:id/comments', auth, upload.array('attachments',3), async (req, res) => {
  ...
  try {
    ...
    if (req.files?.length) {
      for (const file of req.files) {
        await client.query(
          ...
          [req.params.id,comment.id,file.originalname,file.filename,file.mimetype,file.size,req.user.id]
        );
      }
    }
    ...
    if (!internal) {
      ...
      if (ticket?.created_by_email) sendEmail(ticket.created_by_email, emailTemplates.ticketUpdated(ticket,author,'New reply posted',content));
    }
