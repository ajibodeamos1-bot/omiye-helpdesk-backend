...
router.post('/register', auth, requireRole('super_admin'), [
  ...
  body('role').isIn(['care_rep','ict_staff','ict_manager','branch_manager','finance_officer','super_admin']),
  ...
  try {
    ...
    res.status(201).json({ message: 'User created. Welcome email sent.', user: newUser });
  } catch (err) { console.error(err); res.status(500).json({ message: 'Server error' }); }
});
...
router.get('/me', auth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, full_name, email, role, branch, is_active, must_change_password FROM users WHERE id = $1',
      [req.user.id]
    );
...
router.put('/change-password', auth, [
  ...
  try {
    ...
    const sameAsNew = await bcrypt.compare(new_password, result.rows[0].password_hash);
...
router.put('/profile', auth, [body('full_name').trim().notEmpty()], async (req, res) => {
